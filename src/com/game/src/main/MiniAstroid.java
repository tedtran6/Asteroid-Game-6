...
public class MiniAstroid implements Hostile {
	 ...
	 */
	public MiniAstroid(double x, double y, int velX, int velY, SpriteCollection SpCol, Controller c, Game game) {
		this.velX = velX;
		this.velY = velY;
		this.x = x;
		this.y = y;
	...
	public void tick() {
		...
		if(ast_health <= 0){
			c.removeEntity(this);
			c.addEntity(new MiniMineral(x, y, SpCol, c, game));
		}
	...
	public void render(Graphics g) {
		g.drawImage(SpCol.miniAstroid, (int) x, (int) y, null);


			g.setColor(Color.RED);
			((Graphics2D) g).setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f));
			g.fillRect((int) x, (int) y, 10, 1);
